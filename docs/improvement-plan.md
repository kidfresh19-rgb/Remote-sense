...
| # | Item | Seam | Notes |
|---|---|---|---|
| T2.1 | **AgriTrack field-activity correlation**: overlay planting / fertiliser / irrigation / spray logs on the index timeline; correlate interventions with vegetation response | new `rs_activity` package (read-only `ActivityLogPort`) + workspace overlay | **Built and tested offline 2026-06-03** (ADR 0005): `ActivityLogPort` + mock adapter, pure `correlate` (baseline NDVI on/before each activity vs mean NDVI in the response window after, and the delta). Read-only, joined on canonical farm/field id, never stored (invariant 6 intact). The real `gateway` feed is parked on the gateway ⚑ CONFIRM. Workspace overlay endpoint is the thin follow-on. |

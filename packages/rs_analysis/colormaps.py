...
# ⚑ CONFIRM (agronomy review): v1 proposed 2026-06-03, pending agronomist sign-off. Display ranges
# tuned to the values that actually occur over Zimbabwean cropland so the ramp spends its contrast
# on the cropping window, not the mathematical extremes. The colormap names follow the matplotlib /
# rio-tiler convention the tiler uses. Sources: per-index typical-range literature (EOS/Sentinel-Hub
# crop guidance; SAVI L=0.5 range compression; NDRE 0.1-0.6 crop band) reconciled to the dry-to-wet
# season swing of NR II-V (FAO Zimbabwe natural regions). Stops are display-only; index math and the
# stored valid_range are untouched.

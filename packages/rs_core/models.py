...
class Annotation(Base):
    """An analyst's free-text note on a field (L6). Optionally scoped to a pass date; always tied
    to the geometry_version it was written against (invariant 5), so it keeps context across a
    boundary change. Written by an authenticated analyst holding the ANNOTATE permission - the
    shared, team-visible store the workspace was built behind, replacing the browser-local seam."""
